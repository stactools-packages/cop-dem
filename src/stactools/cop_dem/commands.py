...
def create_cop_dem_command(cli):
    ...
    @cop_dem.command(
        "create-item", short_help="Creates a STAC item from an Copernicus DEM tile."
    )
    @click.argument("source")
    @click.argument("destination")
    @click.option(
        "--validate/--no-validate", default=True, help="Validate the item before saving"
    )
    @click.option(
        "--host",
        type=click.Choice(co.COP_DEM_HOST.keys(), case_sensitive=False),
        default=None,
        help="Set PROVIDER HOST",
    )
    def create_item_command(source: str, destination: str, validate: bool, host: str):
        """Creates a STAC Item
    ...
    @cop_dem.command(
        "create-collection", short_help="Creates a STAC Collection for Copernicus DEM."
    )
    @click.argument("product", type=click.Choice(["glo-30", "glo-90"]))
    @click.argument("destination", type=click.Path(exists=True))
    @click.option(
        "-u", "--url", default="", type=str, help="Root url to prepend to all records"
    )
    @click.option(
        "--validate/--no-validate", default=True, help="Validate the item before saving"
    )
    ...
    def create_collection_command(
        destination: str, product: str, url: str, validate: bool, host: str
    ):
